...
for i=0:n2
    ...
    % solver
    [U,A,F,mass,stiff,flux,source,bound] = solver_DG_dirichlet_interface_int(K,N,real,alpha,mu,beta,f,dirichlet,leg_b,leg_d,dx,Edge,E2edge,E2E,E2size,E2bound,normal);
    
    % err
    err(i+1,:) = total_error_int(N,real,Edge,leg_b,U,sol);
...
o = zeros(1,3);
for i=1:3
    tmp = polyfit(lim,-log(err(lim+1,i)')/log(2),1);
    o(i) = tmp(1);
end