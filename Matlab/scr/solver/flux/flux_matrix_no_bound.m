function [flux,bound] = flux_matrix_no_bound(K,N,beta,~,leg_b,dx,leg_d,Edge,E2edge)
...
%         end
        ...
        for i = 0:N
            for j = 0:N
                flux(i+1,j+1) = flux(i+1,j+1) + (- leg_d(k,j,x) )* leg_b(k,i,x);
            end
        end
    else
        %%%%% Interior
...
%         end
        ...
        for i = 0:N
            for j = 0:N
                flux(E +i+1,E +j+1) = flux(E +i+1,E +j+1) + (+ leg_d(k,j,x)) * leg_b(k,i,x);
            end
        end
    else
        %%%%% Interior
